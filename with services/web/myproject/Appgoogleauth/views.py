...
def view_files(request):
    ...
    print(f"Fetching files for user: {current_user}")
    
    # Get fresh data from Firestore
    user_files = FirestoreModel.get_user_uploads(current_user)
    
    print(f"Files found for {current_user}: {len(user_files)}")
    for file in user_files:
        print(f"File: {file['zip_name']} - ID: {file.get('doc_id')} - Uploaded by: {file['user']}")
