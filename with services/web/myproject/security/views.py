...
from .serializers import SecurityModelSerializer
...
class SecurityModelAPIView(APIView):
    def get(self, request):
        try:
            serializer = SecurityModelSerializer()
            ...
            return Response({
                'status': 'error',
                'message': f'Model yüklenirken hata oluştu: {str(e)}'
...
class AnalyzeAPIView(APIView):
    def post(self, request):
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                print(f'Geçici dizin oluşturuldu: {tmpdirname}')
                ...
                # Dosya analizi
                print('Dosya analizi başlatılıyor...')
                language_counts = Counter()
                total_syntax_errors = 0
                syntax_errors_by_language = Counter()
                security_issues = []
                ...
                print(f'Dosya analizi tamamlandı. Toplam dosya: {sum(language_counts.values())}')

                # Get the most common language
                primary_language = language_counts.most_common(1)[0][0] if language_counts else 'Unknown'
                print(f'Ana dil: {primary_language}')

                # Run Bandit and save the report as JSON
                print('Bandit analizi başlatılıyor...')
                report_path = os.path.join(tmpdirname, 'bandit_report.json')
                subprocess.run(['bandit', '-r', tmpdirname, '-f', 'json', '-o', report_path], capture_output=True, text=True)
                print('Bandit analizi tamamlandı')
                ...
                # Extract the "results" section
                results = data.get("results", [])
                print(f'Bulunan güvenlik sorunları: {len(results)}')
                ...
                X_test = project_summary.drop(columns=["folder_name"], errors="ignore")
                predicted_quality = float(quality_model.predict(X_test)[0])
                predicted_security = float(security_model.predict(X_test)[0])
                print(f'Tahminler tamamlandı. Kalite: {predicted_quality}, Güvenlik: {predicted_security}')

                # AI önerileri
                ai_recommendations = [
                    f"Proje ana dili: {primary_language}",
                    f"Toplam sözdizimi hatası: {total_syntax_errors}",
                    f"Güvenlik seviyesi: {'Yüksek' if predicted_security > 0.7 else 'Orta' if predicted_security > 0.4 else 'Düşük'}",
                    f"Yazılım kalitesi: {'Yüksek' if predicted_quality > 0.7 else 'Orta' if predicted_quality > 0.4 else 'Düşük'}"
                ]
                ...
                response_data = {
                    ...
                    'model_info': 'Random Forest Model',
                    'predicted_quality': predicted_quality,
                    'predicted_security': predicted_security,
                    'security_issues': [f"{issue['filename']} - Line {issue['line_number']}: {issue['issue_text']}" for issue in security_issues],
                    'ai_recommendations': ai_recommendations,
                    'primary_language': primary_language,
                    'language_stats': dict(language_counts),
                    'total_syntax_errors': total_syntax_errors,
                    'syntax_errors_by_language': dict(syntax_errors_by_language)
                }
