...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Güvenlik Analizi Uygulaması',
      theme: ThemeData(
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _useTestZip() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Test ZIP dosyası yüklenirken hata oluştu: $e')),
      );
    } finally {
      setState(() {
        _isLoading = false;
  ...
  Future<void> _useVTYSProjectZip() async {
    setState(() {
      _isLoading = true;
    });

    try {
      // VTYS-Project-main.zip dosyasını assets'ten yükle
      final ByteData data = await rootBundle.load('assets/VTYS-Project-main.zip');
      final List<int> bytes = data.buffer.asUint8List();
      
      // Geçici bir dosya oluştur
      final tempDir = await getTemporaryDirectory();
      final tempFile = File('${tempDir.path}/VTYS-Project-main.zip');
      await tempFile.writeAsBytes(bytes);
      
      await _extractAndAnalyzeZip(tempFile.path);
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('VTYS Project ZIP dosyası yüklenirken hata oluştu: $e')),
      );
    } finally {
      setState(() {
        _isLoading = false;
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => AnalysisResultPage(
                  ...
                  predictedQuality: analysisResult['predicted_quality'].toDouble(),
                  securityLevel: analysisResult['security_level'],
                  predictedSecurity: analysisResult['predicted_security'].toDouble(),
                  detailedReport: analysisResult['detailed_report'],
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 20),
                    ElevatedButton.icon(
                      onPressed: _useVTYSProjectZip,
                      icon: const Icon(Icons.analytics),
                      label: const Text('VTYS Project Analiz Et'),
                      style: ElevatedButton.styleFrom(
                        padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 15),
                    ...
                    const SizedBox(height: 20),
                    const Text(
                      'Lütfen analiz etmek istediğiniz ZIP dosyasını seçin veya hazır ZIP\'lerden birini kullanın',
